...
import { FEATURED_PROJECTS, PLAY_NICETIES, projectDeepLink, repositoryLabel } from "../src/studio/projects.js";
...
test("repository-backed projects point at complete GitHub subprojects", () => {
  ...
  assert.ok(projects.every((project) => project.repository.repo === "hara-play"));
  ...
  assert.equal(repositoryLabel(liveValues.repository), "hara-lang/hara-play/samples/live-values");
});
...
test("Peacock Ballroom is a provider-backed world project", () => {
  ...
  const link = new URL(projectDeepLink(project, "/"), "https://play.hara-lang.org/");
...
test("project links carry branch and project path", () => {
  ...
  assert.match(link, /^\/\?/);
  assert.match(link, /repo=hara-lang%2Fhara-play/);
...
test("the editor nicety catalog covers structural editing and kernel feedback", () => {
  const ids = new Set(PLAY_NICETIES.map((feature) => feature.id));
